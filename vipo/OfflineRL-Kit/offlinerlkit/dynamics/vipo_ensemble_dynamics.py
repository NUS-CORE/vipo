...
class VIPOEnsembleDynamics(BaseDynamics):
    def __init__(
        self,
        model: nn.Module,
    ...
    def train(
        ...
        inputs, targets, obss = self.format_samples_for_training(data)
        data_size = inputs.shape[0]
        holdout_size = min(int(data_size * holdout_ratio), 1000)
        train_size = data_size - holdout_size
        train_splits, holdout_splits = torch.utils.data.random_split(range(data_size), (train_size, holdout_size))
        train_inputs, train_targets, train_obss = inputs[train_splits.indices], targets[train_splits.indices], obss[train_splits.indices]
    ...
    def learn(
        ...
        for batch_num in range(int(np.ceil(train_size / batch_size))):
            ...
            with torch.no_grad():
                v_tp1 = self.true_value_net(obs_tp1_det)
                target_v = reward_true_det + discount * v_tp1
            ...
            with torch.no_grad():
                v_model_tp1 = self.ensemble_value_net(obs_tp1_hat)
                target_model_v = r_hat_mean + discount * v_model_tp1   # (E,B)
            ...
            with torch.no_grad():
                ...
                v_model_tp1 = self.ensemble_value_net(obs_tp1_hat)                                   # (E,B)
                pred_return = r_hat_mean + discount * v_model_tp1                                   # (E,B)
