...
from offlinerlkit.dynamics import EnsembleDynamics,VIPOEnsembleDynamics_EXP
...
def train(args=get_args()):
    ...
    dynamics = VIPOEnsembleDynamics_EXP(
        ...
        scaler,
        termination_fn,
        penalty_coef=0.0,                  # 看你要不要用 COMBO 的不确定性惩罚
        uncertainty_mode="aleatoric",
        discount=0.99,
        phi=1e-4,                          # VIPO 正则的权重，大一点/小一点自己再扫
        value_hidden_dims=[256, 256],      # 或者和 dynamics_hidden_dims 一样
        value_lr=3e-4,
    )
